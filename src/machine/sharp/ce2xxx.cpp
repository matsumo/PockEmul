...
Cce2xxx::Cce2xxx(CPObject *parent , Models mod, QString _cfg):CPObject(parent,_cfg)
{
    ...
    case CE201M:    // 8KB Wide RAM card
        ...
        if (getcfgfname().isEmpty()) setcfgfname(QString("ce201m"));
        memsize      = 0x2000;
    ...
    case CE202M:    // 16KB Wide RAM card
        ...
        if (getcfgfname().isEmpty()) setcfgfname(QString("ce202m"));
        memsize      = 0x4000;
            ...
            if (xmlIn->readNextStartElement() && xmlIn->name() == "memory" ) {
                ...
                for (int s=0; s<SlotList.size(); s++)                               // Save Memory
                {
                    ...
                    case CSlot::CUSTOM_ROM:
                        ...
                        qWarning()<<"Load Slot "<<SlotList[s].getLabel();
