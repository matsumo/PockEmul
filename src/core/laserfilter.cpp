...
laserfilter::laserfilter(QObject *parent)
...
//    connect(&decoder, &LaserFilter::decodingStarted,
//            this, &laserfilter::handleDecodingStarted);
//    connect(&decoder, &LaserFilter::decodingFinished,
//            this, &laserfilter::handleDecodingFinished);
//    connect(&decoder, &LaserFilter::tagFound,
//            this, &laserfilter::handleTagFound);
}
...
void laserfilter::handleDecodingStarted()
{
    decoding = true;
...
void laserfilter::handleDecodingFinished(bool succeeded)
{
    qDebug() << "handleDecodingFinished"<<succeeded;
    decoding = false;
//    emit decodingFinished(succeeded, decoder.getProcessTimeOfLastDecoding());
    emit isDecodingChanged();
}

void laserfilter::handleTagFound(QString tag)
{
    qDebug() << "handleTagFound";
    emit tagFound(tag);
}

QVideoFilterRunnable * laserfilter::createFilterRunnable()
{
    return new laserfilterRunnable(this);
}
...
QImage laserfilter::fromBGRAtoARGB(uchar * data, QSize size, QVideoFrame::PixelFormat pixelFormat)
{
...
laserfilterRunnable::laserfilterRunnable(laserfilter * filter)
    : QObject(nullptr)
    , filter(filter)
{
...
QVideoFrame laserfilterRunnable::run(QVideoFrame * input, const QVideoSurfaceFormat &surfaceFormat, RunFlags flags)
{
    ...
    {
        qDebug() << "[laserfilterRunnable] Invalid Input ";
        return * input;
    }
    ...
    filter->processThread = QtConcurrent::run(this, &laserfilterRunnable::processVideoFrameProbed, filter->frame);

    return * input;
}

void laserfilterRunnable::processVideoFrameProbed(SimpleVideoFrame & videoFrame)
{
    ...
    if(image.isNull())
        image = laserfilter::fromBGRAtoARGB((uchar*)videoFrame.data.data(), videoFrame.size, videoFrame.pixelFormat);
    ...
    if(image.isNull())
    {
        qDebug() << "laserfilterRunnable error: Cant create image file to process.";
